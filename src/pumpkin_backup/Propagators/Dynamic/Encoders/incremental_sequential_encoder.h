...
#include <vector>
namespace Pumpkin {
class IncrementalSequentialEncoder : public IEncoder<CardinalityConstraint> {
...
//  void SetSumLiterals(std::vector<BooleanLiteral> sum_lits) override;
  ...
  class Factory : public IEncoder<CardinalityConstraint>::IFactory {
    ...
    IEncoder<CardinalityConstraint> * CallConstructor(CardinalityConstraint &constraint) override {return new IncrementalSequentialEncoder(constraint.literals, constraint.min, constraint.max);};
  };

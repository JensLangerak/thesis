...
int WatchedPseudoBooleanConstraint3::next_log_id_ = 0;

bool CompareByWeight2(const WeightedLiteral &a, WeightedLiteral &b)
{
  return a.weight > b.weight;
}

WatchedPseudoBooleanConstraint3::WatchedPseudoBooleanConstraint3(std::vector<BooleanLiteral> &literals, std::vector<uint32_t> &weights, int max, IEncoder<PseudoBooleanConstraint> *encoder) :
encoder_(encoder), max_(max), current_sum_value(0){
  ...
  std::sort(original_literals_.begin(), original_literals_.end(), CompareByWeight2);
...
WatchedPseudoBooleanConstraint3::ExplainLiteralPropagation(
    BooleanLiteral literal, SolverState &state) {
  return new ExplanationPseudoBooleanConstraint3(this, state, literal);
}
ExplanationPseudoBooleanConstraint3 *
WatchedPseudoBooleanConstraint3::ExplainFailure(SolverState &state) {
  return new ExplanationPseudoBooleanConstraint3(this, state);
}
WatchedPseudoBooleanConstraint3::~WatchedPseudoBooleanConstraint3() {
  if (encoder_ != nullptr)
    delete encoder_;
}
void WatchedPseudoBooleanConstraint3::UpdateCounts(
...
//      lit_decisions_[l.ToPositiveInteger()]++;
  ...
  if (add_partial) {
    if (!encoder_->EncodingAdded()) {
      state.propagator_pseudo_boolean_3_.add_constraints_.push(this);
...
void WatchedPseudoBooleanConstraint3::UpdateConflictCount(SolverState &state) {
//  if (!logged_during_conflict)
//    UpdateLog(state, lit_logs_counts_conflict);
  logged_during_conflict = true;
}
void WatchedPseudoBooleanConstraint3::UpdatePropagateCount(SolverState &state) {
  return;
//  if (!logged_during_conflict)
//    UpdateLog(state, lit_logs_counts_conflict);
  logged_during_conflict = true;
}
void WatchedPseudoBooleanConstraint3::UpdateNotTouchedCount(
    SolverState &state) {
//  if (!logged_during_conflict)
//    UpdateLog(state, lit_logs_counts_not_touched);
}
void WatchedPseudoBooleanConstraint3::UpdateSolutionCount(SolverState &state) {
  return;
  assert(!logged_during_conflict);
//  UpdateLog(state, lit_logs_counts_solution);
}
void WatchedPseudoBooleanConstraint3::LogCounts() {
  return;
...
bool WatchedPseudoBooleanConstraint3::GetLabel(BooleanLiteral l,
                                               std::string &label) {
...
void WatchedPseudoBooleanConstraint3::UpdateNode(BooleanLiteral literal,
                                                 int conflict_id) {

...
#ifndef SIMPLESATSOLVER_SRC_PUMPKIN_PROPAGATORS_DYNAMIC_PSEUDOBOOLEANLOWER_WATCHED_PSEUDO_BOOLEAN_CONSTRAINT_H_
#define SIMPLESATSOLVER_SRC_PUMPKIN_PROPAGATORS_DYNAMIC_PSEUDOBOOLEANLOWER_WATCHED_PSEUDO_BOOLEAN_CONSTRAINT_H_
...
namespace Pumpkin {
class ExplanationPseudoBooleanConstraint3;
class SolverState;
...
class WatchedPseudoBooleanConstraint3 {
public:
  WatchedPseudoBooleanConstraint3(std::vector<BooleanLiteral> &literals, std::vector<uint32_t> &weights, int max, IEncoder<PseudoBooleanConstraint> *encoder);
  ~WatchedPseudoBooleanConstraint3();
  ...
  ExplanationPseudoBooleanConstraint3 *ExplainFailure(SolverState &state); // returns the conjunction that leads to failure

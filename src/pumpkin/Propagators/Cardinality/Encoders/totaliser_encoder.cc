...
namespace Pumpkin {
TotaliserEncoder::TotaliserEncoder(SolverState &solver_state, std::vector<BooleanLiteral> variables,
                                   int min, int max)
...
TotaliserEncoder::Node *TotaliserEncoder::CreateTree(std::vector<BooleanLiteral> variables) {
  ...
  if (n->index == 1) {
    ...
    for (int i = 0; i < variables.size(); i++) {
      n->counting_variables.push_back(BooleanLiteral(solver_state_->CreateNewVariable(), true));
    }

    int m = n->index / 2;
    std::vector<BooleanLiteral> variables_left(variables.begin(), variables.begin() + m);
    std::vector<BooleanLiteral> variables_right(variables.begin() + m, variables.end());

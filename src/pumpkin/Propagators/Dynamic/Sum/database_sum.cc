...
WatchedSumConstraint* DatabaseSum::AddPermanentConstraint(SumConstraint &constraint, SolverState & state) {
    IEncoder<SumConstraint> * encoder =  constraint.encoder_factory->Create(constraint);
    WatchedSumConstraint * watched = new WatchedSumConstraint(constraint.input_literals,constraint.output_literals, encoder);

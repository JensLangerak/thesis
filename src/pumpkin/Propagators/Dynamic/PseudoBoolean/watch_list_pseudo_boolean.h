...
#ifndef SIMPLESATSOLVER_SRC_PUMPKIN_PROPAGATORS_DYNAMIC_PSEUDOBOOLEAN_WATCH_LIST_PSEUDO_BOOLEAN_H_
#define SIMPLESATSOLVER_SRC_PUMPKIN_PROPAGATORS_DYNAMIC_PSEUDOBOOLEAN_WATCH_LIST_PSEUDO_BOOLEAN_H_

#include "../../../Basic Data Structures/boolean_literal.h"
#include "watcher_pseudo_boolean_constraint.h"
#include <cstdint>
namespace Pumpkin {
class WatchListPseudoBoolean2 {
public:
  explicit WatchListPseudoBoolean2(uint64_t num_variables);
  void Add(BooleanLiteral literal, int weight, WatchedPseudoBooleanConstraint2 *constraint); //Adds the clause_ to the watch list of the literal. It assumes clause_ copying is forbidden and internally keeps a pointer to "clause_".
  void Remove(BooleanLiteral literal, WatchedPseudoBooleanConstraint2 *constraint); //Removes the clause_ from the watch list of the literal. It asumes clauses are compared based on the memory locations. Linearly scans through the list until it encounters the clause_, therefore if the index is already know it is better to use the overloaded RemoveClauseFromWatch with an integer as input
  ...
  const std::vector<WatcherPseudoBooleanConstraint2>& operator[](BooleanLiteral literal) const; //returns the reference to the vector of clauses that are watched by the literal
  std::vector<WatcherPseudoBooleanConstraint2>& operator[](BooleanLiteral literal);

  bool IsLiteralWatchingConstraint(BooleanLiteral literal, const WatchedPseudoBooleanConstraint2 *constraint) const;
...
private:
  ...
  std::vector<std::vector<WatcherPseudoBooleanConstraint2> > watch_lists_;
};
} // namespace Pumpkin

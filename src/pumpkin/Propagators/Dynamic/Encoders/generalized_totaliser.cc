...
GeneralizedTotaliser::Encode(SolverState &state) {
...
//    std::cout << root_->counting_variables_weights[i] << " " << (state.assignments_.IsAssignedTrue(root_->counting_variables[i]) ? "T   " : "F   ");
//  }
  int sum =0;
  for (int i = 0; i < variables_.size(); ++i) {
    if (state.assignments_.IsAssignedTrue(variables_[i]))
      sum += weights_[i];
  }
  if (sum > max_ && root_ != nullptr) {
      for (int i = 0; i < root_->counting_variables.size(); i++) {
    std::cout << root_->counting_variables_weights[i] << " " << (state.assignments_.IsAssignedTrue(root_->counting_variables[i]) ? "T   " : "F   ");
  }
  std::cout <<std::endl;
    TreeValid(state, root_);
  }
...
void GeneralizedTotaliser::PrintState(SolverState &state) {}
bool GeneralizedTotaliser::AddLiteral(
  ...
  if (update_node->index == update_node->nr_leafs) {
    ...
    assert(update_node->left != nullptr && update_node->left->index == update_node->left->nr_leafs);
...
GeneralizedTotaliser::Node *
GeneralizedTotaliser::MergeNode(SolverState & state, GeneralizedTotaliser::Node *node_l,
...
//    p->clauses.push_back({~node_l->counting_variables[i], p->counting_variables[index]});
    added_clauses.push_back({~node_l->counting_variables[i], p->counting_variables[index]});
  }
...
//    p->clauses.push_back({~node_r->counting_variables[i], p->counting_variables[index]});
    added_clauses.push_back({~node_r->counting_variables[i], p->counting_variables[index]});
  }
...
//      p->clauses.push_back({~node_l->counting_variables[q], ~node_r->counting_variables[r], p->counting_variables[index]});
      added_clauses.push_back({~node_l->counting_variables[q], ~node_r->counting_variables[r], p->counting_variables[index]});
...
GeneralizedTotaliser::Node *
GeneralizedTotaliser::CreateTree(SolverState &state,
                                 GeneralizedTotaliser::Node *leaf, int start_index) {
...
void GeneralizedTotaliser::UpdateParents(SolverState &state,
                                         GeneralizedTotaliser::Node *child, BooleanLiteral added_lit,
                                         std::vector<std::vector<BooleanLiteral>> &added_clauses) {
...
int GeneralizedTotaliser::AddValueIndex(SolverState& state, GeneralizedTotaliser::Node *n,
                                        int value) {
  assert(value > 0);
...
int GeneralizedTotaliser::TreeValid(SolverState &state,
    ...
    for (int i = 0; i < node->counting_variables.size(); i++) {
      assert(node->counting_variables_weights[i] <= value == state.assignments_.IsAssignedTrue(node->counting_variables[i]));
    }

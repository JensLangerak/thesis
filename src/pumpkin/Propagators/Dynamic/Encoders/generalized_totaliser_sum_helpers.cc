...
#include <stack>
#include <utility>
#include "../Sum/pb_sum_helper.h"
#include "../Sum/Input/propagator_pb_sum_input.h"
#include "../Sum/Output/propagator_pb_sum_output.h"
...
WatchedPbSumConstraint* GeneralizedTotaliserSumHelpers::AddPbSumConstraint(
    ...
    int max, double add_delay, EncodingStrategy encoding_strategy ,
...
//  encoder->encoding_strategy_ = encoding_strategy;
  auto factory = new GeneralizedTotaliserSumNodes::Factory(encoding_strategy, add_delay);
//  encoder_result = encoder;
  PbSumConstraint constraint = PbSumConstraint(input_lits, input_weighs, output_lits, output_weighs);
  WatchedPbSumConstraint * res =
      PbSumHelper::AddConstraint(constraint, state, factory);
  //TODO
  encoder_result = dynamic_cast<GeneralizedTotaliserSumNodes *>(res->encoder_);
  encoder_result->watched_constraint = res;
  return res;

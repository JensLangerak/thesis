...
namespace Pumpkin {
bool PropagatorExtendedGroups::PropagateLiteral(BooleanLiteral true_literal,
                                         SolverState &state) {
...
//  assert(CheckCounts(state));
  ...
  for (; current_index < watchers_true.size(); ++current_index) {
    WatchedExtendedGroupsConstraint *constraint =
...
//    int last_ind = last_index_;
    ...
    for (int i = 0; i <constraint->intput_liters_.size(); ++i) {
      WeightedLiteral l = constraint->intput_liters_[i];
      if (slack >= l.weight)
        break;
...
void PropagatorExtendedGroups::Synchronise(SolverState &state) {
...
PropagatorExtendedGroups::PropagatorExtendedGroups(int64_t num_variables)    : PropagatorDynamic(), pseudo_boolean_database_(num_variables){
}
}

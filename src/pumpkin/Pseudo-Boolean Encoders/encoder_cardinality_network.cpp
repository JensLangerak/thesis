...
std::vector<BooleanLiteral> EncoderCardinalityNetwork::HMerge(const std::vector<BooleanLiteral>& a, const std::vector<BooleanLiteral>& b, SolverState& state)
{
    ...
    for (size_t i = 1; i < c.size()-1; i++) { c[i] = BooleanLiteral(state.CreateNewVariable(), true); }
    c[0] = d[0];
    
    if (2 * e.size() == c.size()) { c.back() = e.back(); }
    else { c.back() = BooleanLiteral(state.CreateNewVariable(), true); }    

    for (size_t i = 1; i < d.size(); i++) { num_clauses_added_last_time_++; state.AddImplication(d[i + 1 - 1], c[2 * i - 1]); }

    for (size_t i = 1; i < e.size(); i++){ num_clauses_added_last_time_++; state.AddImplication(e[i - 1], c[2 * i - 1]); }
    if (2 * e.size() != c.size())
    {
        num_clauses_added_last_time_++;
        size_t m = e.size();
        state.AddImplication(e[m - 1], c[2 * m - 1]);
    }
    ...
    {
        num_clauses_added_last_time_++;
        state.AddTernaryClause(~d[i + 1 - 1], ~e[i - 1], c[2 * i + 1 - 1]);
    }
    if (2 * e.size() != c.size())
    {
        num_clauses_added_last_time_++;
        size_t m = e.size();
        state.AddTernaryClause(~d[m + 1 - 1], ~e[m - 1], c[2 * m + 1 - 1]);
    }
...
{
    BooleanLiteral sum_at_least_one(state.CreateNewVariable(), true);
    BooleanLiteral sum_at_least_two(state.CreateNewVariable(), true);

    state.AddImplication(a, sum_at_least_one);
    state.AddImplication(b, sum_at_least_one);
    state.AddTernaryClause(~a, ~b, sum_at_least_two);

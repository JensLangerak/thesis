...
//private:
	...
	std::vector<BooleanLiteral> present_literals_; //keep tracks of all literals that were used in this constraint. Note that some of these literals might have zero coefficients due to cancellations. Used to make ClearDataStructures() more efficient. note: I am not sure, but I think we can't just remove the zero-coefficient literals from this vector, since I think the algorithms needs to know which literals were used...not sure, something to think about.

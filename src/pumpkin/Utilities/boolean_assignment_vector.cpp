...
bool BooleanAssignmentVector::operator[](BooleanVariable variable) const
{
    runtime_assert(!variable.IsUndefined());
    return assignments_.at(variable.index_);
}

std::vector<bool>::reference BooleanAssignmentVector::operator[](BooleanVariable variable)
{
    runtime_assert(!variable.IsUndefined());
    return assignments_.at(variable.index_);
}

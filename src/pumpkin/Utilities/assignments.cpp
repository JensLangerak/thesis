...
Assignments::Assignments(size_t number_of_variables):
	truth_values_(number_of_variables+1, 2), 
...
{
	assert(!literal.IsUndefined() && truth_values_[literal.Variable().index_] >= 0 && truth_values_[literal.Variable().index_] <= 2);
...
{
	runtime_assert(1 == 2); //I think this method should be removed; root assignment are not decisions not propagations in a standard sense, they should be removed from the formula
...
bool Assignments::IsRootAssignment(BooleanLiteral literal) const
{
	return GetAssignmentLevel(literal) == 0;
}

bool Assignments::IsDecision(BooleanVariable variable) const
{
	return IsAssigned(variable) && GetAssignmentPropagator(variable) == NULL && GetAssignmentLevel(variable) > 0;
}

bool Assignments::IsDecision(BooleanLiteral literal) const
{
...
int Assignments::GetAssignmentLevel(BooleanLiteral literal) const
{
...
PropagatorGeneric* Assignments::GetAssignmentPropagator(BooleanLiteral literal) const
{
	runtime_assert(IsAssignedTrue(literal));
	return info_[literal.VariableIndex()].responsible_propagator;
}

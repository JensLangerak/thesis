...
int64_t LinearBooleanFunctionComplex::GetMaximumWeightSmallerThanThreshold(int64_t threshold) const
{
    ...
    for (BooleanVariable weighted_variable : weighted_literals_)
    {
...
typename std::vector<BooleanVariable>::const_iterator LinearBooleanFunctionComplex::begin() const
{
    return weighted_literals_.begin();
}

typename std::vector<BooleanVariable>::const_iterator LinearBooleanFunctionComplex::end() const
{
...
int64_t LinearBooleanFunctionComplex::ComputeSolutionCost(const BooleanAssignmentVector& solution)
{
    ...
    for (BooleanVariable variable : weighted_literals_)
    {
        if (weights_[variable] > 0 && solution[variable.index_]
            || weights_[variable] < 0 && !solution[variable.index_])
        {
...
{
    BooleanVariable variable = lit.Variable();
    if (variable.index_ + 1 > weights_.Size()) { weights_.Resize(variable.index_ + 1, 0); }
}

...
BooleanLiteral BooleanLiteral::IntToLiteral(int code)
{
	int variable_index = code / 2;
	bool is_positive = code % 2;
	BooleanLiteral reconstructed_literal = BooleanLiteral(BooleanVariable(variable_index), is_positive);
	assert(reconstructed_literal.ToPositiveInteger() == code);
	return reconstructed_literal;
}

BooleanLiteral BooleanLiteral::UndefinedLiteral()
{
